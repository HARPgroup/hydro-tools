...
Groupings[,123:nrow(Lone_segs)] <- NA

Groupings[25,] <- t(Lone_segs)
...
#create groupings for river segs based on major and minor (setting up for 2 different maps) -------------
...
#create group number for river segs
i<-1
x<-1
y<-ncol(Groupings)
...
png(filename=paste0(file_path, "\\LoneSeg_MajMinRiverBasins.png"), 
...
RiverSeg@data$color<- cut(RiverSeg@data$Min,c(0.5,1.5,2.5,3.5,4.5,5.5,6.5,7.5,8.5,9.5,10.5,11.5,12.5, 13.5, 14.5, 15.5, 16.5, 17.5, 18.5, 19.5, 20.5, 21.5), 
                             labels=c('blue', 'blueviolet', 'brown', 'brown1', 'burlywood1', 
                                      ...
                                      #'turquoise4', 'chocolate4', 'gray57', 'black'))
...
dir.create(paste0(file_path), showWarnings = FALSE);
png(filename=paste0(file_path, "\\LoneSegMajRiverBasins.png"), 
    width=1400, height=950, units="px")
...
#find a way to hatch out all lone segs (group = 25) -----------------------------------------
# for minor basins
# SETTING UP FOR LEGEND ----------
Groupingsnum<-nrow(Groupings)-1
GroupingsnumFirst<-round(Groupingsnum/2, digits=0)
...
# GRAPHING -----------------------------------------------------------------
title<-('Greyed Coastals CPB Major-Minor Watershed Basins')

dir.create(paste0(file_path), showWarnings = FALSE);
png(filename=paste0(file_path, "\\Greyed_LoneSegMajMinRiverBasins.png"), 
    width=1400, height=950, units="px")

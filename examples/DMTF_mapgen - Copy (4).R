...
for (w in 1:length(gages[,1])) {
 ...
 gage_w_geom <- geom_sf(data = gage_w_sf,aes(geometry = geom,colour = color_list[1]), shape=19, inherit.aes = FALSE,size=2)
 gage_w_bbox <- st_bbox(gage_w_sf)
 gage_w_bbox <- data.frame(x = gage_w_bbox$xmin, y = gage_w_bbox$ymin)
 gage_w_label <- geom_label_repel(data = gage_w_bbox, aes(x = x, y = y, group = 1, label = intakes$name),size = 2,fill="white",box.padding =1,max.iter=20000)
...
#---------------------------------------------------------------------------------------------------
 wells <- read.csv(paste(base_url,"/drought-well-feature-export",sep=""))
   ...
   for (w in 1:length(wells[,1])) {
     ...
     well_w_geom <- geom_sf(data = well_w_sf,aes(geometry = geom,colour = color_list[2]), shape=17, inherit.aes = FALSE,size=2)
...
finalmap.obj <- map.obj +
                ...
                theme(legend.position = c(0.33, 0.782)
                )
                             
               # scale_shape_manual(name = "Monitoring Sites", 
               #                    labels=c("Surface Water", "Groundwater"),
               #                    values = c(19, 17))
                 # scale_color_manual("lines (means?)",
                 #                    values = c("brown4", "blue"),
                 #                    labels = c("Indicator Streamgage", "Indicator Well"),
                 #                    guide = guide_legend(override.aes = list(fill = NA)))
deqlogo <- draw_image(paste(github_location,'/HARParchive/GIS_layers/HiResDEQLogo.tif',sep=''),scale = 0.175, height = 1, x = -.388, y = -0.413) #LEFT BOTTOM LOGO
...
#   
#   #ggsave(plot = permit_gwvs_map_draw, file = paste0(export_path, "GWPermits_AWRR_2020.pdf",sep = ""), width=6.5, height=4.95) #FINAL MAP SAVES HER###################################################################################################### 
# # THIS SCRIPT CURRENTLY PRODUCES 18 MAP IMAGES (AS PNG OR PDF)
# ######################################################################################################
# library("dataRetrieval")
# library("ggspatial")

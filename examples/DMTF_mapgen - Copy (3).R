...
for (w in 1:length(gages[,1])) {
 ...
 gage_w_geom <- geom_sf(data = gage_w_sf,aes(geometry = geom,colour = color_list[1]), inherit.aes = FALSE,size=2)
...
#---------------------------------------------------------------------------------------------------
 wells <- read.csv(paste(base_url,"/drought-well-feature-export",sep=""))
   ...
   for (w in 1:length(wells[,1])) {
     ...
     well_w_geom <- geom_sf(data = well_w_sf,aes(geometry = geom,colour = color_list[2]), inherit.aes = FALSE,size=2)

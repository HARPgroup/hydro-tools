...
library("tidyr")
library("kableExtra")
## Pulls in foundation_location for onedrive
...
ows_permit_list <- read.csv(file = paste0(foundation_location,"\\OWS\\foundation_datasets\\awrr\\",eyear+1,"\\ows_permit_list.csv")) 
...
mp_all_mgy <- read.csv(file = paste0(foundation_location,"\\OWS\\foundation_datasets\\awrr\\",eyear+1,"\\mp_all_mgy_",eyear-4,"-",eyear,".csv"))
...
write.csv(join_all, paste(foundation_location,"\\OWS\\foundation_datasets\\awrr\\",eyear+1,"\\mp_permitted_",eyear,".csv",sep = ""), row.names = F)

#read static data in
join_all <- read.csv(file = paste(foundation_location,"\\OWS\\foundation_datasets\\awrr\\",eyear+1,"\\mp_permitted_",eyear,".csv",sep = ""))
...
table3w_tex
table3w_tex %>%
  cat(., file = paste(foundation_location,"\\OWS\\Report Development\\Annual Water Resources Report\\October ",eyear+1," Report\\overleaf\\summary_table3.tex",sep = ''))

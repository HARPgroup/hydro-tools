...
region <-sqldf(paste('SELECT *,fips_geom AS geom,
                          CASE
 ...
 layer.sf <- st_as_sf(region, wkt = 'geom')
 class(layer.sf)
 st_crs(layer.sf) = 3850
 
 #layer.gg <- geom_sf(data = layer.sf,aes(fill = factor(col)),lwd=0.4, inherit.aes = FALSE, show.legend =TRUE)
 st_crs(layer.sf)
 #layer_sp <- st_set_crs(layer.sf,"+proj=utm +zone19 +ellps=GRS80 +datum=NAD84")
 # layer_sp <- st_set_crs(layer.sf,"26919")
 # st_crs(layer_sp)

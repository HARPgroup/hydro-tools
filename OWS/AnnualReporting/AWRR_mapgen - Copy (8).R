...
region <-sqldf(paste('SELECT *,
                          CASE
                            WHEN fips_code IN (51191,51167,51169,51173,51520,51185,51720,51105,51027,51051,51195) THEN "khaki1"
                            WHEN fips_code IN (51025,51081,51053,51620,51111,51135,51149,51175,51181,51183,51595) THEN "slateblue2"
                            ELSE "white"
                          END AS col
                        FROM layer_df 
                        ',sep=""))
...
#layer.df <- data.frame(name = "EXAMPLE_NAME",geom = layer_wkt)
 ...
 #layer.gg <- geom_sf(data = layer.sf,aes(geometry = fips_geom),fill = layer.sf$col,lwd=0.4, inherit.aes = FALSE, show.legend = "Region")
 #layer.gg <- geom_sf(data = layer.sf,aes(geometry = fips_geom,fill = factor(col)),lwd=0.4, inherit.aes = FALSE, show.legend = "Region")
 ...
 finalmap.obj <- basemap.obj + layer.gg +
  ...
  theme(legend.position = c(0.2, 0.9)) +
   # scale_fill_manual(values = c("khaki1" = "blue"), 
   #                   guide = guide_legend(override.aes = list(linetype = "blank", shape = NA)))
   
 # scale_fill_manual(name = "Region", values = c("white","khaki1","slateblue2"),
 #                   labels=c("white","khaki1","slateblue2"),
 #                   guide = guide_legend(override.aes = list(shape = c(20,20,20)))) #+
...
# unique(layer.sf$col)
   ...
   scale_fill_manual(name = "Drought Evaluation Regions",
                     #values = c("white","khaki1","slateblue2"),
                     values = c("khaki1","slateblue2","white"),
                     labels = c("Big Sandy","Chowan","white")
                     )

ggsave(plot = finalmap.obj, file = paste0(export_path, "tables_maps/Xfigures/","finalmap.obj.png",sep = ""), width=6.5, height=4.95)

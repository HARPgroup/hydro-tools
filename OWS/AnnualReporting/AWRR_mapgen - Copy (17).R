...
localpath <-"C:/Users/jklei/Desktop/GitHub/HARParchive/GIS_layers"
...
res.df <- data.frame("name"="all","geom"=shp_layer_wkt)
#write.csv(res.df,paste0(export_path, "tables_maps/Xfigures/","MajorReservoirs.csv",sep = ""))
res.sf <- st_as_sf(res.df, wkt = 'geom')
res.gg <- geom_sf(data = res.sf,fill="black",lwd=0.4, inherit.aes = FALSE, show.legend =FALSE)

# 
# WBDF <- baselayers[[which(names(baselayers) == "WBDF")]]
# fips.sf <- st_as_sf(fips_df, wkt = 'geom')
# resy.gg <- geom_sf(data = reservoirs.gg,fill="black",lwd=0.4, inherit.aes = FALSE, show.legend =TRUE)
# colnames(WBDF) 

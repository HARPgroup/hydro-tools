...
# RUN GILES MAP:
map_out <- facmap(facility_hydroid = 72813, 
                  map_zoom = 0.18, #Larger value = more zoomed out
                  basemap_resolution = 12, #Larger value = higher resolution
                  scalebar_dist = 4, #Scale bar distance in miles
                  scalebar_nudge = 0.11,  #Scale bar placement "nudge"
...
outfalls <- data.frame(geom='POINT(-80.757516000000 37.335266000000)')
...
outfall_geom <- geom_sf(data = outfall_sf,aes(geometry = geom,colour = color_list[5]), inherit.aes = FALSE,size=1)
...
gages <- data.frame(name=c('NEW RIVER AT RADFORD, VA','NEW RIVER AT GLEN LYN, VA'),
                    geom=c('POINT(-80.56922499 37.14179254)','POINT(-80.8606347 37.3728986)'))
if (nrow(gages) > 0){
  gage_geom_list <- list()
...
#   leg_labels = c("River Segments","Intake","Well","Transfer","WTP Outfall","USGS Gage")
...
#   leg_labels = c("River Segments","Intake","Well","Transfer","WTP Outfall")
...
leg_values = c("gray30","darkorchid3","orange","black","blue")
leg_labels = c("River Segments","Intake","Well","WTP Outfall","USGS Gage")
leg_linetype = c("solid","blank","blank","blank","blank")
leg_alpha = c(1,1,1,1,1)
leg_shape = c(NA,16,15,16,17)
